...
<?
                        echo "
                            <form class='form-edit-block' name='formEditUser' action=''>
                                <input class='edit-input' type='hidden' name='editInp' value=".$line['id_user'].">
                                <div class='edit-block'>
                                    ...
                                    <div class='grid-column edit-input-block'>
                                        <input id='name' class='edit-input' type='text' name='userName' value=".$line['user_name'].">
                                        <input id='tel' class='edit-input' type='tel' placeholder='+7 (XXX) XXX-XX-XX' maxlength='11' name='userTel' value=".$line['user_tel'].">
                                        <input id='mail' class='edit-input' type='email' name='userMail' value=".$line['user_mail'].">
                                    </div>
                                </div>
...
<?
        ...
        if(isset($_GET['save'])) {
            $sql2 = "UPDATE `Users` SET `user_name` = ?, `user_tel` = ?, `user_mail` = ? WHERE `id_user` = ?";
            $edit = $pdo -> prepare($sql2);
            $edit -> execute(array("$_GET[userName]", "$_GET[userTel]", "$_GET[userMail]", "$_GET[editInp]"));
